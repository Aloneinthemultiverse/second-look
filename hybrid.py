...
BLENDS = [0.0, 0.05, 0.10, 0.20, 0.30, 0.40, 1.0]
...
def main() -> None:
    ...
    m = lgb.train(LGB_P, lgb.Dataset(X_tr, label=y_tr), num_boost_round=300)
    ...
    (config.ARTIFACTS / "hybrid.json").write_text(
        json.dumps({"spearman_all": float(sp), "pearson_all": float(pe_r),
                    "spearman_frauds": float(sp_f), "rescued_frauds": rescued,
                    "blends": {str(k): v for k, v in rows.items()},
                    "best_blend": float(best_b)}, indent=2), encoding="utf-8")
    print(f"\nwrote {config.ARTIFACTS / 'hybrid.json'}")
